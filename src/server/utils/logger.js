...
function formatLogMessage(options, context = "") {
	...
	const logLevel = winstonConfig.colorize(
		options.level,
		options.level.toUpperCase().padEnd(5)
	);
	const meta = isEmptyObjectOrUndefined(options.meta)
		? ""
		: util.format(options.meta);
	return `${colors.dim}${time}${colors.reset} ${logLevel} ${
		colors.dim
	}[${context}]${colors.reset} ${options.message} ${meta}`;
}
