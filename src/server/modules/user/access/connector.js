...
const combine = (features, extractor) =>
	without(
		union(...map(features, (res) => castArray(extractor(res)))),
		undefined
	);
...
class Feature {
	...
	constructor({
		...
		this.createContextFunc = combine(
			arguments,
			(arg) => arg.createContextFunc
		);
		this.htmlHeadComponent = combine(
			arguments,
			(arg) => arg.htmlHeadComponent
		);
	}
